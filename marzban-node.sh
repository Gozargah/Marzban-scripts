...
while [[ $# -gt 0 ]]; do
    ...
    case $key in
    install | update | uninstall | up | down | restart | status | logs | core-update | install-script | edit | uninstall-script)
        COMMAND="$1"
        shift # past argument
        ;;
    --name)
...
update_core_command() {
    ...
    if ! yq eval '.services."marzban-node".volumes[] | select(. == "/var/lib/marzban-node:/var/lib/marzban-node")' "$COMPOSE_FILE" &>/dev/null; then
        yq eval '.services."marzban-node".volumes += "/var/lib/marzban-node:/var/lib/marzban-node"' -i "$COMPOSE_FILE"
    fi
...
usage() {
    ...
    echo "  uninstall-script  Uninstall Marzban-node script"
